...
if [[ -n "$BOOT_CONFIG" ]]; then
  ...
  sudo sed -i 's/^dtoverlay=vc4-fkms-v3d/#dtoverlay=vc4-fkms-v3d  # disabled by setup-player-service.sh/' "$BOOT_CONFIG"

  # Add full KMS overlay if not already present
  if ! sudo grep -q "^dtoverlay=vc4-kms-v3d" "$BOOT_CONFIG"; then
    sudo bash -lc "printf '\n# Added by setup-player-service.sh\ndtoverlay=vc4-kms-v3d\n' >> '$BOOT_CONFIG'"
  fi
...
CONF_SOURCE="${REPO_DIR}/system/lume-browser.conf.template"
...
UNIT_SOURCE="${REPO_DIR}/system/lume-browser.service"
